...
|--------|-----------|-------|-------------|
| scl_o | Output | 8 | I2C clock output |
| scl_i | Input | 8 | I2C clock input (for clock stretching) |
| scl_oe | Output | - | I2C clock output enable (active low in wrapper) |
| sda_o | Output | 9 | I2C data output |
| sda_i | Input | 9 | I2C data input |
| sda_oe | Output | - | I2C data output enable (active low in wrapper) |

**Note**: GPIO 8 and 9 are configured as bidirectional pads for proper I2C open-drain operation.

### I2C Slave Signals (External Test Module)

| Signal | Direction | GPIO# | Description |
|--------|-----------|-------|-------------|
| slave_scl | Input | 8 | Connected to master SCL (shared bus) |
| slave_sda_i | Input | 9 | Connected to master SDA (shared bus) |
| slave_sda_o | Output | 9 | Slave SDA output |
...
| GPIO# | Usage | Reason |
|-------|-------|--------|
| 0-7 | Reserved | Caravel system usage, JTAG, etc. |
| 8 | I2C SCL | I2C clock line |
| 9 | I2C SDA | I2C data line |
| 10-37 | Available | Free for future expansion |
...
```verilog
// Change from GPIO 8,9 to GPIO 10,11
assign scl_i = io_in[10];
assign io_out[10] = scl_o;
assign io_oeb[10] = ~scl_oen;

assign sda_i = io_in[11];
assign io_out[11] = sda_o;
assign io_oeb[11] = ~sda_oen;
```
